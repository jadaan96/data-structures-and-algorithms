...
const biggerThanLuke = (arr) => {
  ...
  const lukeMass = parseInt(luke.mass);
  return arr.filter((character) => {
    const mass = parseInt(character.mass);
    return !isNaN(mass) && mass > lukeMass;
...
const isSecure = (url) => {
  ...
  // Otherwise, return true for secure URLs
  return true;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const detectTicTacToeWin = (board) => {
  ...
  for (let row = 0; row < 3; row++) {
    if (helpCheck(row, 0, row, 1, row, 2, board)) {
      return true;
    }
  }
  ...
  for (let col = 0; col < 3; col++) {
    if (helpCheck(0, col, 1, col, 2, col, board)) {
      return true;
    }
  }

  // Check diagonals
  if (helpCheck(0, 0, 1, 1, 2, 2, board) || helpCheck(0, 2, 1, 1, 2, 0, board)) {
    return true;
  }
