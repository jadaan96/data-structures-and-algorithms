...
const countNumberOfChildren = (arr) => {
  const totalChildren = arr.reduce((acc, curr) => {
    if (curr.children) {
...
const extractStats = (snorlaxData) => {
  const statsObject = snorlaxData.stats.reduce((acc, curr) => {
    acc[curr.stat.name] = curr.baseStat;
    return acc;
  }, {});
  return statsObject;
};
...
const extractChildren = (arr) => {
  const filteredArr = arr.filter((char) => char.name.toLowerCase().includes('a'));
  return filteredArr.reduce((acc, char) => {
    if (char.children) {
      return acc.concat(char.children);
    }
    return acc;
  }, []);
};
