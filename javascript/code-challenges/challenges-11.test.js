...
function transformToLis(obj) {
  ...
  // Map over the keys and create an array of list item strings
  const lis = keys.map((key) => `<li>${key}: ${obj[key]}</li>`);
...
const count = (target, input) => {
  const flattened = input.reduce((acc, curr) => acc.concat(curr), []);
  return flattened.filter(num => num === target).length;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
let findShortest = (data) => {
  return data.reduce((acc, cur) => {
    if (!acc || Number(cur.height) < Number(acc.height)) {
      return cur;
    }
    return acc;
  }, null).name;
